...
/**
 * @file EW_triplet_mass_eigenstates.cpp
 * @brief implementation of the EW_triplet model class
 *
 * Contains the definition of the EW_triplet model class methods
...
#define CLASSNAME EW_triplet_mass_eigenstates
...
CLASSNAME::EW_triplet_mass_eigenstates(const EW_triplet_input_parameters& input_)
   : EW_triplet_soft_parameters(input_)
   ...
   , physical()
   , problems(EW_triplet_info::particle_names)
   ...
   , MVG(0), MHp(0), MFv(Eigen::Array<double,3,1>::Zero()), MFc(0), MFn(0), MAh
      ...
      3,1>::Zero()), MFe(Eigen::Array<double,3,1>::Zero()), MVWp(0), MVP(0), MVZ(0
      )
   ...
   , PhaseFc(1,0), PhaseFn(1,0)

{
}

CLASSNAME::~EW_triplet_mass_eigenstates()
{
}
...
const EW_triplet_physical& CLASSNAME::get_physical() const
{
   return physical;
}

EW_triplet_physical& CLASSNAME::get_physical()
{
   return physical;
}

void CLASSNAME::set_physical(const EW_triplet_physical& physical_)
{
   physical = physical_;
}

const Problems<EW_triplet_info::NUMBER_OF_PARTICLES>& CLASSNAME::get_problems() const
{
   return problems;
}

Problems<EW_triplet_info::NUMBER_OF_PARTICLES>& CLASSNAME::get_problems()
{
   return problems;
}
...
int CLASSNAME::tadpole_equations(const gsl_vector* x, void* params, gsl_vector* f)
{
   ...
   EW_triplet_mass_eigenstates* model = ewsb_args->model;
...
void CLASSNAME::print(std::ostream& ostr) const
{
   ostr << "========================================\n"
           "EW_triplet\n"
           "========================================\n";
   EW_triplet_soft_parameters::print(ostr);
   ostr << "----------------------------------------\n"
...
{
   if (PHYSICAL(Mhh) < 0.) problems.flag_tachyon(EW_triplet_info::hh);
...
{
   EW_triplet_soft_parameters::clear();
   clear_DRbar_parameters();
   physical.clear();
   problems.clear();
}
...
void CLASSNAME::run_to(double scale, double eps)
{
   if (eps < 0.0)
      eps = precision;
   EW_triplet_soft_parameters::run_to(scale, eps);
}
...
void CLASSNAME::calculate_MHp()
{
   ...
   if (MHp < 0.) {
      problems.flag_tachyon(EW_triplet_info::Hp);
   }
...
void CLASSNAME::calculate_MAh()
{
   ...
   if (MAh < 0.) {
      problems.flag_tachyon(EW_triplet_info::Ah);
   }
...
void CLASSNAME::calculate_Mhh()
{
   ...
   if (Mhh < 0.) {
      problems.flag_tachyon(EW_triplet_info::hh);
   }
...
void CLASSNAME::calculate_MFd()
{
   ...
   problems.flag_bad_mass(EW_triplet_info::Fd, eigenvalue_error >
      precision * Abs(MFd(0)));
#else
...
void CLASSNAME::calculate_MFu()
{
   ...
   problems.flag_bad_mass(EW_triplet_info::Fu, eigenvalue_error >
      precision * Abs(MFu(0)));
#else
...
void CLASSNAME::calculate_MFe()
{
   ...
   problems.flag_bad_mass(EW_triplet_info::Fe, eigenvalue_error >
      precision * Abs(MFe(0)));
#else
...
void CLASSNAME::calculate_MVWp()
{
   ...
   if (MVWp < 0.) {
      problems.flag_tachyon(EW_triplet_info::VWp);
   }
...
std::complex<double> CLASSNAME::self_energy_Fn_1(double p ) const
{
   ...
   result += -8*MFc*(-0.5 + B0(p,MFc,MVWp))*Conj(CpFnconjVWpFcPR())*
      CpFnconjVWpFcPL();
...
std::complex<double> CLASSNAME::self_energy_Fn_PR(double p ) const
{
   ...
   result += -2*AbsSqr(CpFnconjVWpFcPL())*(0.5 + B1(p,MFc,MVWp));
...
std::complex<double> CLASSNAME::self_energy_Fn_PL(double p ) const
{
   ...
   result += -2*AbsSqr(CpFnconjVWpFcPR())*(0.5 + B1(p,MFc,MVWp));
...
void CLASSNAME::calculate_MVG_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::VG);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::VG);
}
...
void CLASSNAME::calculate_MFv_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::Fv);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::Fv);
}
...
void CLASSNAME::calculate_MFc_pole()
{
   ...
   unsigned iteration = 0;
   double diff = 0.0;
   decltype(MFc) old_MFc(MFc), new_MFc(MFc);

   do {
      const double M_tree(MFc);
      const double p = old_MFc;
      ...
      new_MFc = PHYSICAL(MFc);
      diff = MaxRelDiff(new_MFc, old_MFc);
      old_MFc = new_MFc;
      iteration++;
   } while (diff > precision
            && iteration < number_of_mass_iterations);
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::Fc);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::Fc);
}
...
void CLASSNAME::calculate_MFn_pole()
{
   ...
   unsigned iteration = 0;
   double diff = 0.0;
   decltype(MFn) old_MFn(MFn), new_MFn(MFn);

   do {
      const double M_tree(MFn);
      const double p = old_MFn;
      ...
      new_MFn = PHYSICAL(MFn);
      diff = MaxRelDiff(new_MFn, old_MFn);
      old_MFn = new_MFn;
      iteration++;
   } while (diff > precision
            && iteration < number_of_mass_iterations);
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::Fn);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::Fn);
}

void CLASSNAME::calculate_Mhh_pole()
{
   if (!force_output && problems.is_tachyon(EW_triplet_info::hh))
      return;
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::hh);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::hh);
}
...
void CLASSNAME::calculate_MVP_pole()
{
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::VP);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::VP);
}

void CLASSNAME::calculate_MVZ_pole()
{
   if (!force_output && problems.is_tachyon(EW_triplet_info::VZ))
      return;
   ...
   do {
      ...
      if (mass_sqr < 0.)
         problems.flag_tachyon(EW_triplet_info::VZ);
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::VZ);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::VZ);
}
...
void CLASSNAME::calculate_MFd_pole()
{
   ...
   do {
      ...
      for (unsigned es = 0; es < 3; ++es) {
         ...
         fs_svd(M_loop, eigen_values, mix_Vd, mix_Ud,
            eigenvalue_error);
         problems.flag_bad_mass(EW_triplet_info::Fd,
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::Fd);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::Fd);
}
...
void CLASSNAME::calculate_MFu_pole()
{
   ...
   do {
      ...
      for (unsigned es = 0; es < 3; ++es) {
         ...
         fs_svd(M_loop, eigen_values, mix_Vu, mix_Uu,
            eigenvalue_error);
         problems.flag_bad_mass(EW_triplet_info::Fu,
            eigenvalue_error > precision * Abs(eigen_values(0)));
      #else
         fs_svd(M_loop, eigen_values, mix_Vu, mix_Uu);
      #endif
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::Fu);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::Fu);
}
...
void CLASSNAME::calculate_MFe_pole()
{
   ...
   do {
      ...
      for (unsigned es = 0; es < 3; ++es) {
         ...
         fs_svd(M_loop, eigen_values, mix_Ve, mix_Ue,
            eigenvalue_error);
         problems.flag_bad_mass(EW_triplet_info::Fe,
            eigenvalue_error > precision * Abs(eigen_values(0)));
      #else
         fs_svd(M_loop, eigen_values, mix_Ve, mix_Ue);
      #endif
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::Fe);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::Fe);
}

void CLASSNAME::calculate_MVWp_pole()
{
   if (!force_output && problems.is_tachyon(EW_triplet_info::VWp))
      return;
   ...
   if (diff > precision)
      problems.flag_no_pole_mass_convergence(EW_triplet_info::VWp);
   else
      problems.unflag_no_pole_mass_convergence(EW_triplet_info::VWp);
}
...
std::ostream& operator<<(std::ostream& ostr, const EW_triplet_mass_eigenstates& model)
{
   model.print(ostr);
   return ostr;
}
