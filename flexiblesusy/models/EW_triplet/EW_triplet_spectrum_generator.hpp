...
#ifndef EW_triplet_SPECTRUM_GENERATOR_H
#define EW_triplet_SPECTRUM_GENERATOR_H

#include "EW_triplet_spectrum_generator_interface.hpp"
...
#include "EW_triplet_two_scale_initial_guesser.hpp"
...
template <class T>
class EW_triplet_spectrum_generator
   : public EW_triplet_spectrum_generator_interface<T> {
public:
   EW_triplet_spectrum_generator()
      : EW_triplet_spectrum_generator_interface<T>()
      , high_scale(0.)
      , susy_scale(0.)
      , low_scale(0.)
   {}
   virtual ~EW_triplet_spectrum_generator() {}

   double get_high_scale() const { return high_scale; }
   double get_susy_scale() const { return susy_scale; }
   double get_low_scale()  const { return low_scale;  }

   virtual void run(const softsusy::QedQcd&, const EW_triplet_input_parameters&);
   void write_running_couplings(const std::string& filename = "EW_triplet_rgflow.dat") const;
 ...
 */
template <class T>
void EW_triplet_spectrum_generator<T>::run(const softsusy::QedQcd& qedqcd,
                                const EW_triplet_input_parameters& input)
{
   EW_triplet<T>& model = this->model;
   model.clear();
   ...
   EW_triplet_high_scale_constraint<T> high_scale_constraint(&model);
   EW_triplet_susy_scale_constraint<T> susy_scale_constraint(&model, qedqcd);
   EW_triplet_low_scale_constraint<T>  low_scale_constraint(&model, qedqcd);
   ...
   EW_triplet_convergence_tester<T> convergence_tester(
   ...
   EW_triplet_initial_guesser<T> initial_guesser(&model, qedqcd,
 ...
 */
template <class T>
void EW_triplet_spectrum_generator<T>::write_running_couplings(
   const std::string& filename) const
{
   EW_triplet_spectrum_generator_interface<T>::write_running_couplings(filename, low_scale, high_scale);
}
