...
#ifndef EW_triplet_SPECTRUM_GENERATOR_INTERFACE_H
#define EW_triplet_SPECTRUM_GENERATOR_INTERFACE_H
...
template <class T>
class EW_triplet_spectrum_generator_interface {
public:
   EW_triplet_spectrum_generator_interface()
      : model()
      , settings()
      , parameter_output_scale(0.)
      , reached_precision(std::numeric_limits<double>::infinity())
   {}
   virtual ~EW_triplet_spectrum_generator_interface() {}

   const EW_triplet<T>& get_model() const { return model; }
   EW_triplet<T>& get_model() { return model; }
   ...
   virtual void run(const softsusy::QedQcd&, const EW_triplet_input_parameters&) = 0;
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "EW_triplet_spectrum.dat") const;
...
protected:
   ...
   void translate_exception_to_problem(EW_triplet<T>& model);
};
 ...
 */
template <class T>
void EW_triplet_spectrum_generator_interface<T>::set_settings(
 ...
 */
template <class T>
void EW_triplet_spectrum_generator_interface<T>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   EW_triplet_mass_eigenstates tmp_model(model);
   try {
   ...
   EW_triplet_parameter_getter parameter_getter;
   Coupling_monitor<EW_triplet_mass_eigenstates, EW_triplet_parameter_getter>
 ...
 */
template <class T>
void EW_triplet_spectrum_generator_interface<T>::write_spectrum(const std::string& filename) const
{
   EW_triplet_spectrum_plotter plotter;
 ...
 */
template <class T>
void EW_triplet_spectrum_generator_interface<T>::translate_exception_to_problem(EW_triplet<T>& model)
{
   try {
      throw;
      ...
      const std::string parameter_name =
         parameter_index < 0 ? "Q" : EW_triplet_info::parameter_names[parameter_index];
