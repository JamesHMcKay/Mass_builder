...
#ifndef EW_triplet_UTILITIES_H
#define EW_triplet_UTILITIES_H
...
struct EW_triplet_observables;
class Physical_input;

class EW_triplet_parameter_getter {
public:
   Eigen::ArrayXd get_parameters(const EW_triplet_mass_eigenstates& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names() const {
      using namespace EW_triplet_info;
...
class EW_triplet_spectrum_plotter {
public:
   void extract_spectrum(const EW_triplet_mass_eigenstates&);
...
template <class Scalar, int M, int N>
std::valarray<double> EW_triplet_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
...
void to_database(
   const std::string&,
   const EW_triplet_mass_eigenstates&,
   const softsusy::QedQcd* qedqcd = 0,
   const Physical_input* physical_input = 0,
   const EW_triplet_observables* observables = 0);
...
EW_triplet_mass_eigenstates from_database(
   ...
   EW_triplet_observables* observables = 0);
