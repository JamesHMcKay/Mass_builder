...
#include "EW_triplet_standard_model_two_scale_matching.hpp"
...
#define CLASSNAME EW_triplet_standard_model_Matching<Two_scale>

namespace flexiblesusy {

CLASSNAME::EW_triplet_standard_model_Matching()
...
void CLASSNAME::set_models(Two_scale_model* low, Two_scale_model* high)
{
   ...
   model = cast_model<EW_triplet<Two_scale>*>(high);
}
...
void CLASSNAME::match_high_to_low_scale_model()
{
   ...
   if (model->get_thresholds() && loop_order_down)
      EW_triplet_standard_model_matching::match_high_to_low_scale_model(*eft, *model, loop_order_down, higgs_idx);
   else
      EW_triplet_standard_model_matching::match_high_to_low_scale_model_tree_level(*eft, *model, higgs_idx);
}
...
void CLASSNAME::match_low_to_high_scale_model()
{
   ...
   if (model->get_thresholds() && loop_order_up)
      EW_triplet_standard_model_matching::match_low_to_high_scale_model(*model, *eft, loop_order_up);
   else
      EW_triplet_standard_model_matching::match_low_to_high_scale_model_tree_level(*model, *eft);
}
