...
MDM_effective_couplings::MDM_effective_couplings(
   const MDM_mass_eigenstates& model_,
...
void MDM_effective_couplings::calculate_effective_couplings()
{
...
void MDM_effective_couplings::set_model(const MDM_mass_eigenstates& model_)
{
   model = model_;
   copy_mixing_matrices_from_model();
}

void MDM_effective_couplings::copy_mixing_matrices_from_model()
{
   Vd = PHYSICAL(Vd);
   Ud = PHYSICAL(Ud);
   Vu = PHYSICAL(Vu);
...
standard_model::Standard_model MDM_effective_couplings::initialise_SM() const
{
...
void MDM_effective_couplings::run_SM_strong_coupling_to(standard_model::Standard_model sm, double m)
{
   sm.run_to(m);
...
std::complex<double> MDM_effective_couplings::scalar_scalar_qcd_factor(double m_decay, double m_loop) const
{
...
std::complex<double> MDM_effective_couplings::scalar_fermion_qcd_factor(double m_decay, double m_loop) const
{
   std::complex<double> result(1.0, 0.0);
...
std::complex<double> MDM_effective_couplings::pseudoscalar_fermion_qcd_factor(double m_decay, double m_loop) const
{
   std::complex<double> result(1.0, 0.0);
...
double MDM_effective_couplings::number_of_active_flavours(double m) const
{
...
double MDM_effective_couplings::scalar_scaling_factor(double m) const
{
...
double MDM_effective_couplings::pseudoscalar_scaling_factor(double m) const
{
   const double Nf = number_of_active_flavours(m);
   const double mtpole = qedqcd.displayPoleMt();
   const double l = Log(Sqr(m) / Sqr(mtpole));
...
double MDM_effective_couplings::get_hhVPVP_partial_width() const
{
   const double mass = PHYSICAL(Mhh);
   return 0.0049735919716217296 * Power(mass, 3.0) * AbsSqr(eff_CphhVPVP);
}

double MDM_effective_couplings::get_hhVGVG_partial_width() const
{
   const double mass = PHYSICAL(Mhh);
   return 0.039788735772973836 * Power(mass, 3.0) * AbsSqr(eff_CphhVGVG);
}

double MDM_effective_couplings::get_AhVPVP_partial_width() const
{
   const double mass = PHYSICAL(MAh);
   return 0.0049735919716217296 * Power(mass, 3.0) * AbsSqr(eff_CpAhVPVP);
}

double MDM_effective_couplings::get_AhVGVG_partial_width() const
{
   const double mass = PHYSICAL(MAh);
   return 0.039788735772973836 * Power(mass, 3.0) * AbsSqr(eff_CpAhVGVG);
}

std::complex<double> MDM_effective_couplings::CpFdhhbarFdPL(unsigned gt1, unsigned gt3) const
{
...
std::complex<double> MDM_effective_couplings::CpFuhhbarFuPL(unsigned gt1, unsigned gt3) const
{
...
std::complex<double> MDM_effective_couplings::CpFehhbarFePL(unsigned gt1, unsigned gt3) const
{
...
double MDM_effective_couplings::CphhVWpconjVWp() const
{
...
std::complex<double> MDM_effective_couplings::CpAhFdbarFdPL(unsigned gt2, unsigned gt3) const
{
   const auto Yd = MODELPARAMETER(Yd);
...
std::complex<double> MDM_effective_couplings::CpAhFubarFuPL(unsigned gt2, unsigned gt3) const
{
   const auto Yu = MODELPARAMETER(Yu);
...
std::complex<double> MDM_effective_couplings::CpAhFebarFePL(unsigned gt2, unsigned gt3) const
{
   const auto Ye = MODELPARAMETER(Ye);
...
void MDM_effective_couplings::calculate_eff_CphhVPVP()
{
...
void MDM_effective_couplings::calculate_eff_CphhVGVG()
{
   const auto g3 = MODELPARAMETER(g3);
   const auto MFd = PHYSICAL(MFd);
   const auto MFu = PHYSICAL(MFu);
...
void MDM_effective_couplings::calculate_eff_CpAhVPVP()
{
   const auto MFd = PHYSICAL(MFd);
   const auto MFu = PHYSICAL(MFu);
   const auto MFe = PHYSICAL(MFe);
...
void MDM_effective_couplings::calculate_eff_CpAhVGVG()
{
   const auto g3 = MODELPARAMETER(g3);
   const auto MFd = PHYSICAL(MFd);
   const auto MFu = PHYSICAL(MFu);
