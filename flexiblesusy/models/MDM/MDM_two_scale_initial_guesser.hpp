...
#ifndef MDM_TWO_SCALE_INITIAL_GUESSER_H
#define MDM_TWO_SCALE_INITIAL_GUESSER_H

#include "MDM_initial_guesser.hpp"
...
/**
 * @class MDM_initial_guesser<Two_scale>
 * @brief initial guesser for the MDM
 */

template<>
class MDM_initial_guesser<Two_scale> : public Initial_guesser<Two_scale> {
public:
   MDM_initial_guesser(MDM<Two_scale>*,
                               const softsusy::QedQcd&,
                               const MDM_low_scale_constraint<Two_scale>&,
                               const MDM_susy_scale_constraint<Two_scale>&,
                               const MDM_high_scale_constraint<Two_scale>&);
   virtual ~MDM_initial_guesser();
   virtual void guess(); ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   MDM<Two_scale>* model; ///< pointer to model class
   ...
   MDM_low_scale_constraint<Two_scale> low_constraint;
   MDM_susy_scale_constraint<Two_scale> susy_constraint;
   MDM_high_scale_constraint<Two_scale> high_constraint;
