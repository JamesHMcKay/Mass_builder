...
// File generated at Fri 13 Oct 2017 19:49:41
 ...
 * CP-even Higgs boson mass of the MDM.
 *
 * @param sm Standard Model
 * @param model BSM model
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
void MDM_standard_model_matching::match_high_to_low_scale_model_tree_level(
   Standard_model& sm, MDM_mass_eigenstates& model, unsigned idx)
{
   model.calculate_DRbar_masses();
 ...
 * CP-even Higgs boson mass of the MDM by requiring that the
 ...
 * @param idx Higgs index (in mass ordered Higgs multiplet)
 */
void MDM_standard_model_matching::match_high_to_low_scale_model(
   Standard_model& sm, MDM_mass_eigenstates& model, unsigned loop_order, unsigned idx)
{
   if (loop_order == 0) {
   ...
   Loop_order_setter<MDM_mass_eigenstates> los_model(model, loop_order);
 ...
 * lightest CP-even Higgs boson mass of the MDM by requiring
 ...
 * the MDM at the tree level from the known Standard Model
 * couplings and the SM vev.
 */
void MDM_standard_model_matching::match_low_to_high_scale_model_tree_level(
   MDM_mass_eigenstates& model, Standard_model& sm)
{
   sm.calculate_DRbar_masses();
   model.set_g1(sm.get_g1()*standard_model_info::normalization_g1/MDM_info::normalization_g1);
   model.set_g2(sm.get_g2()*standard_model_info::normalization_g2/MDM_info::normalization_g2);
   model.set_g3(sm.get_g3()*standard_model_info::normalization_g3/MDM_info::normalization_g3);

   {
      MDM_mass_eigenstates* MODEL = &model;
...
static void calculate_SM_pole_masses(MDM_mass_eigenstates& model, Standard_model& sm)
{
#ifdef ENABLE_THREADS
   MDM_mass_eigenstates* obj_ptr = &model;
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the MDM at the 1-loop level from the known Standard Model
 * couplings and the SM vev.
 */
void MDM_standard_model_matching::match_low_to_high_scale_model(
   MDM_mass_eigenstates& model, Standard_model& sm, unsigned loop_order)
{
   if (loop_order == 0) {
...
/**
 * Calculates the gauge and Yukawa couplings and the SM-like VEV in
 * the MDM at the current loop level from the known Standard
   ...
   MDM_mass_eigenstates& model, Standard_model& sm)
{
   ...
   if (mZ2_1L < 0.)
      model.get_problems().flag_tachyon(MDM_info::VZ);
   else
      model.get_problems().unflag_tachyon(MDM_info::VZ);

   if (mW2_1L < 0.)
      model.get_problems().flag_tachyon(MDM_info::VWp);
   else
      model.get_problems().unflag_tachyon(MDM_info::VWp);



   // define and apply 1L-matched gauge parameters
   const double g1_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) * mZ2_1L / mW2_1L) / MDM_info::normalization_g1;
   const double g2_1L = AbsSqrt(4. * Pi * alpha_em * (1. + delta_alpha_em) / (1. - mW2_1L/mZ2_1L)) / MDM_info::normalization_g2;
   const double g3_1L = AbsSqrt(4. * Pi * alpha_s * (1. + delta_alpha_s)) / MDM_info::normalization_g3;
   ...
   {
      MDM_mass_eigenstates* MODEL = &model;
      const double VEV = 2. * AbsSqrt(mZ2_1L/(Sqr(g1_1L*MDM_info::normalization_g1) + Sqr(g2_1L*MDM_info::normalization_g2)));
