...
/**
 * @file MDM_two_scale_model_slha.hpp
 * @brief contains wrapper class for model class in SLHA convention
 */

// File generated at Fri 1 Sep 2017 15:29:04

#ifndef MDM_TWO_SCALE_SLHA_H
#define MDM_TWO_SCALE_SLHA_H
...
/**
 * @class MDM_slha<Two_scale>
 * @brief model class wrapper in SLHA convention
 */

template<>
class MDM_slha<Two_scale> : public MDM<Two_scale> {
public:
   explicit MDM_slha(const MDM_input_parameters& input_ = MDM_input_parameters());
   explicit MDM_slha(const MDM<Two_scale>&, bool do_convert_masses_to_slha = true);
   virtual ~MDM_slha();
   ...
   const MDM_physical& get_physical_slha() const; ///< returns pole masses to SLHA convention
   MDM_physical& get_physical_slha(); ///< returns pole masses to SLHA convention
   ...
   double get_MFg_pole_slha() const { return PHYSICAL_SLHA(MFg); }
...
private:
   MDM_physical physical_slha; ///< contains the pole masses and mixings in slha convention
