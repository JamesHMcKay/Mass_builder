...
// File generated at Mon 27 Nov 2017 21:10:43
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSMDMMessage", 1);
...
struct Model_data {
   ...
   MDM_input_parameters input;     ///< model input parameters
   ...
   MDM_slha<algorithm_type> model; ///< running parameters and pole masses
};
...
void put_spectrum(const MDM_slha<algorithm_type>& model, MLINK link)
{
   MLPutFunction(link, "List", 53);
   ...
   MLPutRuleTo(link, MODELPARAMETER(MFg), "Fg", {"M"});
   ...
   MLPutRuleTo(link, PHYSICALPARAMETER(MFg), "Fg", {"Pole", "M"});
...
void put_observables(const MDM_observables& observables, MLINK link)
{
...
void check_spectrum(const Model_data& data, MLINK link)
{
   ...
   if (problems.have_problem()) {
      ...
      put_message(link, "FSMDMCalculateSpectrum", "error", msg.str());
   }

   if (problems.have_warning()) {
      std::ostringstream msg;
      problems.print_warnings(msg);
      put_message(link, "FSMDMCalculateSpectrum", "warning", msg.str());
   }
...
void calculate_spectrum(Model_data& data, MLINK link)
{
   ...
   try {
      ...
      put_message(link, "FSMDMCalculateSpectrum", "error", e.what());
      throw EInvalidSpectrum();
   }
   ...
   data.model = MDM_slha<algorithm_type>(
...
DLLEXPORT int FSMDMGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMGetSettings"))
...
DLLEXPORT int FSMDMGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMDMGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMDMCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace MDM_librarylink;
   ...
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   ...
   if (handle == handles.end()) {
      std::cerr << "Error: FSMDMSet: Unknown handle: "
                << hid << std::endl;
      return LIBRARY_FUNCTION_ERROR;
   }
...
DLLEXPORT int FSMDMCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSMDMCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace MDM_librarylink;

   if (!check_number_of_args(link, 1, "FSMDMCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.model.get_scale() == 0.) {
         put_message(link,
            "FSMDMCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSMDMCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSMDMCalculateObservables", "error", e.what());
      put_error_output(link);
   }
