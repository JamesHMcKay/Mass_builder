...
void MDM_spectrum_plotter::extract_spectrum(const MDM_mass_eigenstates& model)
{
   spectrum.clear();
   scale = model.get_scale();

   spectrum.push_back(TParticle("Fc", "\\chi^{+}", to_valarray(PHYSICAL(MFc))));
   spectrum.push_back(TParticle("Fg", "\\chi^{++}", to_valarray(PHYSICAL(MFg))));
...
void MDM_spectrum_plotter::write_to_file(const std::string& file_name) const
{
   if (spectrum.empty())
      return;

   std::ofstream filestr(file_name, std::ios::out);
   VERBOSE_MSG("MDM_spectrum_plotter::write_to_file: opening file: "
               << file_name);
   if (filestr.fail()) {
      ERROR("MDM_spectrum_plotter::write_to_file: can't open file "
            << file_name);
      return;
   }
   ...
   filestr.close();
   VERBOSE_MSG("MDM_spectrum_plotter::write_to_file: file written: "
               << file_name);
}

void MDM_spectrum_plotter::write_spectrum(const TSpectrum& spectrum, std::ofstream& filestr) const
{
   if (!filestr.good()) {
      ERROR("MDM_spectrum_plotter::write_spectrum: "
...
std::valarray<double> MDM_spectrum_plotter::to_valarray(double v)
{
 ...
 */
void to_database(
   const std::string& file_name, const MDM_mass_eigenstates& model,
   ...
   const MDM_observables* observables)
{
   ...
   append(names, MDM_parameter_getter().get_input_parameter_names());
   append(values, model.get_input().get());

   // fill DR-bar parameters
   append(names, MDM_parameter_getter().get_parameter_names());
   append(values, model.get());

   // fill pole masses and mixings
   append(names, MDM_parameter_getter().get_mass_names());
   append(names, MDM_parameter_getter().get_mixing_names());
   ...
   if (MDM_observables::NUMBER_OF_OBSERVABLES > 0 && observables) {
      append(names, MDM_observables::get_names());
...
MDM_mass_eigenstates from_database(
   ...
   Physical_input* physical_input, MDM_observables* observables)
{
   using utilities::append;

   MDM_mass_eigenstates model;
   const std::size_t number_of_parameters = model.get_number_of_parameters();
   const std::size_t number_of_masses = MDM_parameter_getter().get_number_of_masses();
   const std::size_t number_of_mixings = MDM_info::NUMBER_OF_MIXINGS;
   const std::size_t number_of_input_parameters = MDM_info::NUMBER_OF_INPUT_PARAMETERS;
   ...
   const std::size_t number_of_observables =
      (observables ? MDM_observables::NUMBER_OF_OBSERVABLES : 0u);
