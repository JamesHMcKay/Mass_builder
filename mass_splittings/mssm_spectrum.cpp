...
namespace extra_TSIL_interface_MSSM
{
#include TSIL_PATH
  using namespace std;
  TSIL_DATA bar;  
...
double MSSM_spectrum::add_1loop_der(Data &data)
{
	
	return extra_TSIL_interface_MSSM::add_derivatives(data);
	
}


double MSSM_spectrum::iterative_ms_bar_mass(Data data, string particle)
{
...
void MSSM_spectrum::compute_spectra_MB_1loop()
{
  Self_energy se;
...
// determine MSbar parameters
void MSSM_spectrum::compute_spectra_MB_2loop()
{
  Self_energy se;
  
  // matching (?) of SM to Wino model
  data.alpha = data.alpha*( 1.0-real(extra_TSIL_interface_MSSM::gammagamma_Chi(data,data.Q)) /pow(data.Q,2) );
...
bool MSSM_spectrum::compute_spectra_flexiblesusy()
{
  ...
  const std::string slha_file="flexiblesusy/models/MSSM/LesHouches.in.MSSM";
  ...
  spectrum_generator.run(oneset, input);
	...
	MSSM_slha<Two_scale> model(spectrum_generator.get_model());
  ...
  //data.sw = Sin(model.get_ZZ(0,0)); // not used
	...
	data.SE_1["F11_g1"] = model.get_MChi_pole_slha(0) - data.MChi;
	data.SE_1["F12_g1"] = model.get_MCha_pole_slha(0) - data.MChi;
  
  return error;
}


void MSSM_spectrum::compute_tsil()
{
	Self_energy se;
	se.run_tsil(data);	
	
	// add derivatives of 1-loop self energies
	data.SE_2["F11_g1"] = data.SE_2["F11_g1"] +  extra_TSIL_interface_MSSM::F11_der(data);
	data.SE_2["F12_g1"] = data.SE_2["F12_g1"] +  extra_TSIL_interface_MSSM::F12_der(data);	
...
double MSSM_spectrum::get_deltam()
{
...
double MSSM_spectrum::get_neutral_mass()
{
