...
kappa=1/(16\[Pi]^2);
Bnn = 2 - Log[-p^2] + MassBuilderEpsilon*(   -\[Zeta]/2 + (1/2)* Log[-p^2]^2 -  2 Log[-p^2] + 4 );
Get[FileNameJoin[{path, "/models/QED/output/math_data_V1_1_1.mx"}]];
SE0V =SelfEnergyFinite*kappa;

Get[FileNameJoin[{path, "/models/QED/output/math_data_V1_1_1c.mx"}]];
ct0V =SelfEnergyFinite*kappa;

Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_1_1.mx"}]];
SE0 =SelfEnergyFinite*kappa;

Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_1_1c.mx"}]];
ct0 =SelfEnergyFinite*kappa;
Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_1_2.mx"}]];
SE3 =SelfEnergyFinite*kappa^2;

Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_2_2.mx"}]];
SE2 =SelfEnergyFinite*kappa^2;

Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_5_2.mx"}]];
SE5 =SelfEnergyFinite*kappa^2;

Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_1_2c.mx"}]];
ct2 = SelfEnergyFinite*kappa^2;

Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_2_2c.mx"}]];
ct3 =SelfEnergyFinite*kappa^2;

Get[FileNameJoin[{path, "/models/QED/output/math_data_F02_g1_3_2c.mx"}]];
...
/. MassBuilderP^2 -> p^2 /. MassBuilderP -> Momentum[p] /.  MassBuilderEpsilon -> \[Epsilon] /. MassBuilderZeta -> \[Zeta] /. MassBuilderAe[0] -> 0 /.  Pair[Momentum[p], Momentum[p]] -> p^2 /.  TBI[4, p^2, {{1, 0}, {1, 0}}] -> I*(2 - Log[-p^2]) \
/.TJI[4, p^2, {{1, 0}, {1, 0}, {1, 0}}] -> (13 p^2/8 -p^2 Log[-p^2]/2) /. Momentum[p, D] -> p /.D -> 4 - 2 \[Epsilon] /. Pair[p, p] -> p^2 /.MassBuilderBe[0, 0] -> 0;



(* ::Subsection:: *)
(* Obtain the one-loop counter-terms *)


eq1 = Coefficient[ Coefficient[ct0, \[Epsilon], -1] +  Coefficient[SE0, \[Epsilon], -1], p, 1] /. MassBuilderCTZ1 -> 0
Print["We take Z1 = -EL^2"];
Print["The other counter-term couplings are calculated to be:"]
sol1 = Solve[{eq1 == 0}, {Z2}];
eq1V = Coefficient[Coefficient[Series[ct0V, {\[Epsilon], 0, 0}], \[Epsilon], -1] + Coefficient[Series[SE0V, {\[Epsilon], 0, 0}], \[Epsilon], -1], p,2] /. MassBuilderCTZ1 -> 0
sol2 = Solve[{eq1V == 0}, {Z3}];
Set @@@ sol1[[1]];
Set @@@ sol2[[1]];
...
(* ::Subsection:: *)
(* Obtain the two-loop counter-terms *)


div1 = FullSimplify[Series[ct1, {\[Epsilon], 0, 0}] + Series[SE3, {\[Epsilon], 0, -1}] /. Z1 -> EL^2];
div2 = FullSimplify[Series[ct2, {\[Epsilon], 0, 0}] + Series[SE5, {\[Epsilon], 0, -1}] /. Z2 -> -EL^2];
div3 = FullSimplify[Series[ct3, {\[Epsilon], 0, 0}] + Series[SE2, {\[Epsilon], 0, -1}] /. Z2 -> -EL^2];
div4 = FullSimplify[Series[ct0, {\[Epsilon], 0, -1}] + Series[SE0, {\[Epsilon], 0, -1}] /. Z2 -> -EL^2];
...
/. TBI[4,  Pair[Momentum[p], Momentum[p]], {{1, 0}, {1, 0}}] ->  I*(Bnn) /. MassBuilderP^2 -> p^2 /. MassBuilderP -> Momentum[p] /.  MassBuilderEpsilon -> \[Epsilon] /. \
   MassBuilderZeta -> \[Zeta] /. MassBuilderAe[0] -> 0 /.  Pair[Momentum[p], Momentum[p]] -> p^2 /.  TJI[4, p^2, {{1, 0}, {1, 0}, {1, 0}}] -> (13 p^2/8 -  p^2 Log[-p^2]/2) /. Momentum[p, D] -> p /.  D -> 4 - 2 \[Epsilon] /. Pair[p, p] -> p^2 /.  MassBuilderBe[0, 0] -> 0 /. D -> 4 - 2 \[Epsilon];
SE = FullSimplify[Series[SE, {\[Epsilon], 0, -1}] /. Z1 -> EL^2 /. Z2 -> -EL^2];
eq1 = Coefficient[Series[SE, {\[Epsilon], 0, 0}], \[Epsilon], -1]
eq1 = Coefficient[Series[SE, {\[Epsilon], 0, 0}], \[Epsilon], -2]
