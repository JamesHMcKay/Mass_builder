...
void print_vertex_tex(ofstream &myfile, std::string particle_1,std::string particle_2,std::string particle_3,Options options)
{
  ...
  else
  {
    ...
    if (type == "c" )
    {
      myfile<< "The requested counter-term vertex does not exist, or there has been an error in the calculation.\n";
    }
    else
    {
      myfile<< "The requested vertex does not exist, or there has been an error in the calculation.\n";
    }
    myfile<<"\\end{minipage}\n";
  }
...
void print_vertices(Options options)
{
  ...
  << "\\title{\\lstinline{"<<options.model<<"}}\n"
  << "\\author{\\textsf{Mass Builder generated file}}\n"
  << "%\\date{\\today}\n"
